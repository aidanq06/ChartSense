...
async function fetchUniverse() {
    ...
    const { rows } = await pool.query(`select distinct wi.symbol
     from watchlist_items wi
     join watchlists w on w.id = wi.watchlist_id
     where wi.symbol is not null`);
    const symbols = Array.from(new Set([...baseline, ...rows.map((r) => r.symbol)]));
    return symbols;
}
async function fetchQuotes(symbols) {
    ...
    for (const symbol of symbols) {
        try {
            ...
            if (!res.ok)
                throw new Error(`quote ${symbol} ${res.status}`);
...
async function upsertQuotes(quotes) {
    ...
    try {
        ...
        for (const q of quotes) {
            ...
            await client.query(`insert into quotes_latest (symbol, price, open, high, low, previous_close, change, change_percent, volume, ts)
