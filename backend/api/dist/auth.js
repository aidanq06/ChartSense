...
export async function createSession(pool, userId, refreshToken) {
    const hash = await bcrypt.hash(refreshToken, BCRYPT_ROUNDS);
    const expiresAt = new Date(Date.now() + REFRESH_TTL_DAYS * 24 * 60 * 60 * 1000);
    const { rows } = await pool.query('insert into auth_sessions (user_id, refresh_token_hash, expires_at) values ($1, $2, $3) returning id', [userId, hash, expiresAt]);
    return rows[0].id;
}
export async function revokeSessionByToken(pool, userId, refreshToken) {
    ...
    for (const r of rows) {
        const match = await bcrypt.compare(refreshToken, r.refresh_token_hash);
        if (match) {
...
export async function findUserIdByRefreshToken(pool, refreshToken) {
