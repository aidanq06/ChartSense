...
app.addHook('preHandler', async (request) => {
  const openRoutes = new Set<string>([
    '/healthz',
    '/v1/auth/register',
    '/v1/auth/login',
    '/v1/auth/refresh',
  ...
  if (openRoutes.has(route)) return;
  const auth = request.headers['authorization'];
  if (auth && auth.startsWith('Bearer ')) {
    const token = auth.slice(7);
    const claims = verifyAccessToken(token);
    if (!claims?.sub) throw app.httpErrors.unauthorized('Invalid token');
    (request as any).userId = claims.sub;
    return;
  }
  const devUser = request.headers['x-user-id'] as string | undefined;
  if (devUser) {
    (request as any).userId = devUser;
    return;
  }
...
async function getUserChatLimit(userId: string): Promise<number> {
  const sql = `
...
async function requireAndIncrementDailyChatAllowance(userId: string) {
  const limit = await getUserChatLimit(userId);
  // Ensure row exists with the correct limit
  await pool.query(
    'insert into ai_chat_usage_daily (user_id, day, used, daily_limit) values ($1, current_date, 0, $2) on conflict (user_id, day) do update set daily_limit = excluded.daily_limit',
    [userId, limit]
  );
  // Increment if under limit
  const { rows, rowCount } = await pool.query(
    'update ai_chat_usage_daily set used = used + 1 where user_id = $1 and day = current_date and used < daily_limit returning used, daily_limit',
    [userId]
  );
  if (rowCount === 0) {
    ...
    const r = cur[0];
    if (!r) throw new Error('quota_check_failed');
...
app.post('/v1/auth/register', async (request) => {
  ...
  const body = schema.parse((request as any).body ?? {});
...
app.post('/v1/auth/login', async (request, reply) => {
  ...
  if (!rows.length || !rows[0].password_hash) return reply.unauthorized('Invalid credentials');
  const ok = await verifyPassword(rows[0].password_hash as string, body.password);
...
app.post('/v1/auth/refresh', async (request, reply) => {
  ...
  if (!userId) return reply.unauthorized('Invalid refresh token');
...
app.post('/v1/watchlists/:id/items', async (request: FastifyRequest<{ Params: { id: string }, Body: { symbol: string, position?: number } }>, reply: FastifyReply) => {
  ...
  if (!w.length) return reply.notFound('Watchlist not found');
  await pool.query(
    'insert into watchlist_items (watchlist_id, symbol, position) values ($1, $2, coalesce($3, 0)) on conflict (watchlist_id, symbol) do update set position = excluded.position',
    [watchlistId, symbol, position ?? 0]
  );
  return { ok: true };
});

app.delete('/v1/watchlists/:id/items/:symbol', async (request: FastifyRequest<{ Params: { id: string, symbol: string } }>) => {
  const userId = (request as any).userId as string;
...
// Alerts
app.post('/v1/alerts', async (request: FastifyRequest<{ Body: { symbol: string, type: string, threshold: number, window_minutes?: number, channel?: string[] } }>) => {
  const userId = (request as any).userId as string;
  const { symbol, type, threshold, window_minutes, channel } = request.body;
  const { rows } = await pool.query(
    'insert into alert_rules (user_id, symbol, type, threshold, window_minutes, channel) values ($1, $2, $3::alert_type, $4, $5, coalesce($6, \'{push}\')) returning id',
    [userId, symbol, type, threshold, window_minutes ?? null, channel ?? ['push']]
  );
  return { id: rows[0].id };
});

// Market data (served from cache tables)
app.get('/v1/market/quote', async (request: FastifyRequest<{ Querystring: { symbol: string } }>, reply) => {
  const symbol = request.query.symbol;
  if (!symbol) return reply.badRequest('symbol required');
  const { rows } = await pool.query('select * from quotes_latest where symbol = $1', [symbol]);
  if (!rows.length) return reply.notFound('quote not found');
  return rows[0];
});

app.get('/v1/market/candles', async (request: FastifyRequest<{ Querystring: { symbol: string, timeframe?: string, from?: string, to?: string } }>, reply) => {
  const { symbol, timeframe = '5m', from, to } = request.query;
  if (!symbol) return reply.badRequest('symbol required');
  const table = timeframe === '1d' ? 'candles_1d' : 'candles_5m';
  const clauses: string[] = ['symbol = $1'];
  const params: any[] = [symbol];
  if (from) { clauses.push(timeframe === '1d' ? 'day >= $2::date' : 'ts >= $2::timestamptz'); params.push(from); }
  if (to) { clauses.push(timeframe === '1d' ? 'day <= $3::date' : 'ts <= $3::timestamptz'); params.push(to); }
...
app.post('/v1/ai/chat', async (request: FastifyRequest<{ Body: { message: string } }>) => {
  const userId = (request as any).userId as string;
...
app.put('/v1/notification-settings', async (request) => {
  ...
  await pool.query(
    `insert into notification_settings (user_id, push_enabled, email_enabled, quiet_hours_start, quiet_hours_end)
    ...
    [userId, push_enabled ?? null, email_enabled ?? null, quiet_hours_start ?? null, quiet_hours_end ?? null]
  );
  return { ok: true };
});
...
app.post('/v1/devices', async (request) => {
  ...
  const userId = (request as any).userId as string;
  await pool.query(
    `insert into devices (user_id, platform, push_token, locale)
     values ($1, $2, $3, $4)
     on conflict (push_token) do update set user_id = excluded.user_id, platform = excluded.platform, locale = excluded.locale`,
    [userId, body.platform, body.push_token, body.locale ?? null]
  );
  return { ok: true };
});
...
app.get('/v1/me/subscription', async (request) => {
  const userId = (request as any).userId as string;
  const { rows } = await pool.query(
    `select tier, status, expires_at from subscriptions where user_id = $1 order by coalesce(renewed_at, created_at) desc limit 1`,
    [userId]
  );
...
app.post('/v1/me/subscription', async (request) => {
  ...
  const { rows } = await pool.query(
    `insert into subscriptions (user_id, tier, provider, product_id, status, expires_at, renewed_at)
     values ($1,$2,$3,$4,$5,$6, now())
     returning id`,
    [userId, tier, provider, product_id ?? null, status, expires_at ?? null]
  );
  return { id: rows[0].id };
});
